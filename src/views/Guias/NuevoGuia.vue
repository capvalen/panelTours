	...
	guiaStore.guardar(nuevo)
		.then(resp => {
			if (parseInt(resp.id) > 0)
				Swal.fire('Guía creado', `El guía "${nuevo.nombre}" ha sido creado`, 'success')
					.then(() => {
						window.location.href = '/guia/perfil/' + resp.id
					})
			else
				Swal.fire('Error', 'Error al crear guía', 'error')
		})
		.catch(error => {
			console.error(error)
			Swal.fire('Error', 'Error al crear guía', 'error')
...
<template>
	...
	<div class="row mb-5">
		<div class="col-8 mx-auto">
			<div class="d-grid">
				<button type="submit" class="btn btn-primary" @click="guardar()">Generar nuevo guía</button>
			</div>
		</div>
	</div>
</template>
